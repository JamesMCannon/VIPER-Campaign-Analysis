# VLF receiver calibration script
# James Cannon, 6/17/2021
# Adapted from calibrate_LF.py by
...
def calibrate_2ch(file1, file2):
    ...
    caltone_length = 10.0 # seconds
    noise_length = 10.0 # seconds
    
    # ---------- Find caltone region ---------------
    # (you can comment this out and enter it manually if it's not working right)
    caltone_start_sec = 115
    noise_start_sec = 50
    ...
    geom = input('Antenna geometry? (square or tri): ')
    ant_AWG = int(input('Antenna AWG: '))
    ant_baseline = float(input('Antenna baseline (cm): ')) # cm
    Na = int(input('Antenna number of turns: '))
    # geom = 'square'
    # ant_AWG = 18
    # ant_baseline = 81 # cm
...
#     print(Ra, La, Sba, Sea, Fca, Aa)
    ...
    Rd = Ra       # For when there's no dummy loop
    Ld = La       
    ...
    for spacing in [10.00e6/4/m, 10.24e6/4/m, 250.244]:
        ...
        num_bins = 10
        peakvals1 = np.sqrt(np.array([np.sum(pow(np.abs(caltonefft1[p-num_bins:p+num_bins]),2)) for p in peak_inds]))
    ...
    for spacing in [10.00e6/4/m, 10.24e6/4/m, 250.244]:
        ...
        num_bins = 10
        # Square sum ~ 10 bins on either side of the center frequency:
        peakvals2 = np.sqrt(np.array([np.sum(pow(np.abs(caltonefft2[p-num_bins:p+num_bins]),2)) for p in peak_inds]))
    ...
    Omega = 2*np.pi*FrequencySpacing1*np.arange(1,201,FrequencySpacing1)
    ...
    Zd = Za                 # For when there's no dummy loop
    ...
    freq_vec_old = np.arange(0, (200*FrequencySpacing1/1000), FrequencySpacing1/10000)
    freq_vec,step = np.linspace(0, (200*FrequencySpacing1/1000) - 1,1999,retstep=True)

    print(step)
    print(FrequencySpacing1)
    print((freq_vec.shape))
    print((freq_vec_old.shape))
...
if __name__ == '__main__':
    ''' VLF antenna calibration script:
    Usage: python calibrate_LF.py <datafile containing a blip of calibration tones>
    
    This script will:  
        - Ask you for antenna information (AWG (16), baseline(260), number of turns (12))
